...
class HealthReportAnalyzer:
    ...
    async def analyze_report(
        self, file_object=None, text_content: str = None
        ...
        try:
            ...
            async def process_chunk(chunk_text):
                ...
                pos_prompt = (
                    self.agent_prompts["positive_analyzer"]
                ...
                neg_prompt = (
                    self.agent_prompts["negative_analyzer"]
                    + "\n\nIMPORTANT PRIVACY INSTRUCTION: Anonymize all output."
                )
            ...
            if file_object:
                ...
                for i, page in enumerate(pdf_reader.pages):
                    ...
                    if total_processed_length > MAX_TEXT_LENGTH:
                        logger.warning(
                            f"Report exceeded {MAX_TEXT_LENGTH} chars. Stopping extraction to prevent OOM."
                        )
                        break
            ...
            # Run Summary Agent
            sum_prompt = (
                self.agent_prompts["summary_agent"]
                + "\n\nIMPORTANT PRIVACY INSTRUCTION: Anonymize all output."
            )
            ...
            rec_prompt = (
                self.agent_prompts["recommendation_agent"]
                + "\n\nIMPORTANT PRIVACY INSTRUCTION: Anonymize all output."
            )
    ...
    async def extract_abnormal_conditions(self, report_text: str) -> List[str]:
        ...
        try:
            ...
            conditions = [
                cond.strip()
                for cond in conditions_text.split("\n")
    ...
    async def generate_diet_plan(self, report_text: str) -> str:
        ...
        try:
            ...
            diet_plan = await self._run_agent(
                diet_system_prompt,
                f"Create a personalized diet plan for these conditions: {' '.join(conditions)}",
            )
