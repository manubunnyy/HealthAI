    ...
    async def process_file(
        ...
        try:
            if progress_callback:
                progress_callback(0.2, f"Processing {file_name}")
            ...
            doc = ProcessedDocument(
                filename=file_name, content=content, doc_type=doc_type, summary=summary
            )
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Extract all text from this medical image. Return ONLY the extracted text.",
                            },
                            {
                                "type": "image_url",
                                "image_url": {
...
Keep each suggestion under 5 words. Total response must be under 50 words."""
    ...
    async def _run_agent(
        self, input_text: str, context: str, query: str, chat_history: str
    ) -> str:
        """Run agent using Groq API"""
        try:
            formatted_system_prompt = self.system_prompt.format(
                context=context, query=query, chat_history=chat_history
            )
    ...
    async def generate_diet_plan(
        self, query: str, context: str, chat_history: str
    ) -> Optional[Dict]:
        """Generate a diet plan based on the conversation"""
        try:
            query_lower = query.lower()
            if (
                query_lower in ["hi", "hello", "hey", "hi there", "hello there"]
                and len(query_lower) < 10
            ):
                return None

            is_vegetarian = (
                "vegetarian" in query_lower
                or "vegan" in query_lower
            ...
            if "kidney" in query_lower and (
                "stone" in query_lower or "stones" in query_lower
            ):
            ...
            if any(
                ...
                try:
                    ...
                    diet_plan = DietPlan(
                        ...
                        lunch=diet_data.get(
                            ...
                            (
                                ...
                                if diet_preference == "non_veg"
                                else "Lentil soup with salad"
                            ),
                        ),
                        dinner=diet_data.get(
                            "dinner",
                            (
                                "Baked fish with vegetables"
                                if diet_preference == "non_veg"
                                else "Vegetable stir fry with tofu"
                            ),
                        ),
                    ...
                    default_condition = (
                        "Fever" if "fever" in query_lower else "General Recovery"
                    )
                    default_plan = DietPlan(
                        breakfast=(
                            "Oatmeal with fruits"
                            if diet_preference == "veg"
                            else "Eggs with whole grain toast"
                        ),
                        lunch=(
                            "Lentil soup with salad"
                            if diet_preference == "veg"
                            else "Chicken soup with vegetables"
                        ),
                        dinner=(
                            "Vegetable stir fry with tofu"
                            if diet_preference == "veg"
                            else "Baked fish with vegetables"
                        ),
                        snacks=(
                            "Yogurt with nuts"
                            if diet_preference == "veg"
                            else "Greek yogurt with nuts"
                        ),
            ...
            if any(
                term in query_lower for term in ["fever", "sick", "ill", "symptoms"]
            ):
                condition = "Fever" if "fever" in query_lower else "Recovery"
                fallback_plan = DietPlan(
                    breakfast="Light porridge or oatmeal",
                    lunch=(
                        "Vegetable soup" if diet_preference == "veg" else "Chicken soup"
                    ),
                    dinner=(
                        "Rice with lentils"
                        if diet_preference == "veg"
                        else "Rice with boiled chicken"
                    ),
...
For simple queries (like greetings), respond in one short sentence.""",
    ...
    async def process_documents(
        self, files: List[tuple], status_callback: Callable = None
    ) -> bool:
        """
        ...
        try:
            ...
            if status_callback:
                status_callback(
                    "document_processor", "error", 0, "Document processing failed"
                )
            return False

        except Exception as e:
            if status_callback:
                status_callback("document_processor", "error", 0, str(e))
            return False

    async def process_query(
        self, query: str, status_callback: Callable = None
        ...
        try:
            ...
            main_response = await self._get_agent_response(
                "main_agent", query, context, chat_history_str
            )
            ...
            for agent_name in specialist_agents:
                if status_callback:
                    status_callback(
                        ...
                        "working",
                        0.2,
                        f"Running {agent_name.split('_')[0]} analysis",
                    )

                response = await self._get_agent_response(
                    agent_name, query, context, chat_history_str
                )
            ...
            diet_response = await self.diet_agent.generate_diet_plan(
                query, context, chat_history_str
            )
            responses["diet_plan"] = diet_response
            if status_callback:
                status_callback("diet_agent", "completed", 1.0, "Diet plan generated")

            if status_callback:
                status_callback(
                    "synthesis_agent", "working", 0.5, "Synthesizing insights"
                )
            final_response = await self._synthesize_responses(
                query, context, chat_history_str, responses
            )
            responses["synthesis_agent"] = final_response
            if status_callback:
                status_callback(
                    "synthesis_agent", "completed", 1.0, "Response synthesis complete"
                )
