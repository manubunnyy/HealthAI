...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await axios.post(
        "https://healthai-b6y2.onrender.com/chat/query",
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        await axios.post(
          "https://healthai-b6y2.onrender.com/chat/upload",
          formData,
        );
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content: `Successfully processed ${e.target.files?.length} document(s). You can now ask questions about them.`,
          },
        ]);
      } catch (error) {
        ...
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: "Failed to upload documents." },
        ]);
      }
    }
  };
    ...
    <div className="max-w-4xl mx-auto h-[calc(100dvh-8rem)] flex flex-col">
      <header className="mb-6">
        ...
        <p className="text-gray-400">
          Chat with AI about your health concerns and documents
        </p>
      </header>
                ...
                {msg.type === "diet_plan" && msg.data && (
                  <div className="mt-4 bg-glass-100 rounded-xl p-4 border border-yellow-500/30">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <h4 className="text-green-400 text-sm font-semibold mb-1">
                          Vegetarian
                        </h4>
